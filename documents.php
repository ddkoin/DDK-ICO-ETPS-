<?php require_once('func/config.php'); 
checkUser($main_conn);


$CSS.=<<<css
<link href="assets/plugins/form-daterangepicker/daterangepicker-bs3.css" type="text/css" rel="stylesheet"> 
css;
$JS_FILE.=<<<js
<script src="assets/plugins/shufflejs/modernizr.custom.min.js"></script>  <!-- Dependency for Shuffle.js -->
<script src="assets/plugins/shufflejs/jquery.shuffle.min.js"></script>    <!-- Shuffle.js -->

<script src="assets/demo/demo-extras-gallery.js"></script>
<script src="assets/plugins/form-daterangepicker/daterangepicker.js"></script>
<script src="assets/plugins/form-daterangepicker/moment.min.js"></script>
<script src="js/fxerp.js?v=2"></script>
js;
$JS=<<<js

wrap_js_function();

js;
?>
<h2 class="page-title">Documents </h2>
 
<section class="widget">

              

                <div class="row">



<div class="col-lg-6">
<h3>Download Document ETPS</h3>

<table class="table table-striped">
                            <tbody>
                            
                                <tr>
                                <td><a href="https://etpswallet.gold/index_devhaziq.php">ETPS Marketing Plan</a><br></td>
                                <td><center> <a href="https://etpswallet.gold/pdf/latest_etps_marketing_Plan.pdf" target="_blank"> <button class="btn btn-primary btn-xs "><i class="fa fa-qrcode" aria-hidden="true"></i> Download</button></a></center></td>
                              </tr>
                    
                               <tr>
                                <td>Scan your ETPS cert with this QR CODE apps<br><br>
                                <img src="img/qrcode.png" alt="Logo"></td>
                                <td></td>
                              </tr>
                              
                            </tbody>
                        </table>
                </div>

            </section>
    	
<?php


 $MainContent = ob_get_contents();
 ob_end_clean();
 
 include("master.php");

?>